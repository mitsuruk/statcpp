...
ヘッダーをインクルードする方法は2つあります：

### 方法1: 名前空間プレフィックス付きでインクルード（推奨）
...
この方法はヘッダーがどのライブラリに属しているかが明確になるため、推奨されます。

### 方法2: statcpp ディレクトリをインクルードパスに追加
...
この方法ではインクルード文が短くなりますが、インクルードパスの指定が必要です。
