 ...
 * @brief Nonparametric test functions
 *
 * Provides distribution-free statistical tests including Wilcoxon test,
 * Mann-Whitney U test, and Kruskal-Wallis test.
 * Also includes normality tests (Shapiro-Wilk, Kolmogorov-Smirnov) and
 * homogeneity of variance tests (Levene, Bartlett).
 *
 * @note Tie detection: Tied values are identified using exact floating-point
 *       equality (operator==). This is appropriate for observed data values
 *       (integers, fixed-precision decimals) where identical values have
 *       identical bit representations. This is consistent with R's behavior.
 *       Users should avoid passing values computed from floating-point
 *       arithmetic (e.g., sqrt(2)*sqrt(2) vs 2.0) as they may not be
 *       recognized as ties.
 */
...
/**
 * @brief Compute ranks with tie handling
 *
 * Assigns ranks to input data. Elements with the same value are assigned
 * average ranks.
 *
 * @tparam Iterator Input iterator type
 * @param first Beginning iterator of range
 * @param last Ending iterator of range
...
/**
 * @brief Compute tie group sizes from sorted data
 *
 * Returns a vector of tie group sizes t_j, where each t_j > 1 represents
 * a group of tied values. Used for tie corrections in nonparametric tests.
 *
...
/**
 * @brief Perform Shapiro-Wilk test
 *
 * Tests whether data follows a normal distribution.
 * Uses Royston's algorithm for approximation.
 * Optimized for sample sizes n <= 50, but supports up to n <= 5000.
 *
 * @tparam Iterator Input iterator type
 * @param first Beginning iterator of range
 * @param last Ending iterator of range
 * @return test_result Test result containing W statistic, p-value, and sample size
 *
 * @throws std::invalid_argument If there are fewer than 3 elements
 * @throws std::invalid_argument If there are more than 5000 elements
 * @throws std::invalid_argument If variance is zero
 *
 ...
 * @param first Beginning iterator of range
 * @param last Ending iterator of range
 * @return test_result Test result containing D statistic, p-value, and sample size
 *
...
/**
 * @brief Perform Levene test (homogeneity of variance test)
 *
 * Tests whether variances are equal across multiple groups.
 * Uses the median-based Brown-Forsythe version, which is robust to
 * departures from normality.
 *
 * @param groups Vector of vectors containing data for each group
 * @return test_result Test result containing F statistic, p-value, and degrees of freedom
 *
 * @throws std::invalid_argument If there are fewer than 2 groups
 * @throws std::invalid_argument If any group has fewer than 2 elements
 *
...
inline test_result levene_test(const std::vector<std::vector<double>>& groups)
{
    ...
    // Guard: if all deviations are zero (all values identical within each group),
    // variances are trivially equal → F = 0, p = 1
    if (ss_within == 0.0) {
...
/**
 * @brief Perform Bartlett test (homogeneity of variance test)
 *
 * Tests whether variances are equal across multiple groups.
 * Based on likelihood ratio test assuming normal distribution.
 * For data that may not follow normal distribution, Levene test is recommended.
 *
 * @param groups Vector of vectors containing data for each group
 * @return test_result Test result containing chi-square statistic, p-value, and degrees of freedom
 *
 * @throws std::invalid_argument If there are fewer than 2 groups
 * @throws std::invalid_argument If any group has fewer than 2 elements
 * @throws std::invalid_argument If any group has zero or negative variance
 *
 * @note Null hypothesis: All groups have equal variance
...
/**
 * @brief Perform Wilcoxon signed-rank test (one-sample)
 *
 * Tests whether the median of data equals a specified value.
 * A nonparametric test that does not assume normal distribution.
 * Uses normal approximation (with continuity correction) for p-value calculation.
 *
 * @tparam Iterator Input iterator type
 * @param first Beginning iterator of range
 * @param last Ending iterator of range
 * @param mu0 Median under null hypothesis (default: 0.0)
 * @param alt Type of alternative hypothesis (default: two-sided)
 * @return test_result Test result containing W+ statistic (sum of ranks for positive differences), p-value, and sample size of non-zero differences
 *
 * @throws std::invalid_argument If there are fewer than 2 elements
                                       ...
                                       alternative_hypothesis alt = alternative_hypothesis::two_sided)
{
    ...
    // Sort absolute differences to compute tie groups
    ...
    // Subtract tie correction: sum(t^3 - t) / 48
...
/**
 * @brief Perform Mann-Whitney U test (two-sample)
 *
 * Tests whether two independent samples come from the same distribution.
 * Also known as Wilcoxon rank-sum test, a nonparametric test that does not assume normal distribution.
 ...
 * @param alt Type of alternative hypothesis (default: two-sided)
 * @return test_result Test result containing U1 statistic, p-value, and total sample size
 *
                                 ...
                                 alternative_hypothesis alt = alternative_hypothesis::two_sided)
{
    ...
    // Tie correction: compute tie groups from sorted combined values
    {
        ...
        // combined is already sorted by value
        ...
        // Corrected variance: n1*n2/(12*N*(N-1)) * (N^3 - N - tie_sum)
        // which equals n1*n2/12 * (N+1) - n1*n2/(12*N*(N-1)) * tie_sum
...
/**
 * @brief Perform Kruskal-Wallis test (k-sample)
 *
 * Tests whether three or more independent samples come from the same distribution.
 * The nonparametric version of one-way ANOVA that does not assume normal distribution.
 * Uses chi-square approximation for p-value calculation.
 *
 * @param groups Vector of vectors containing data for each group
 * @return test_result Test result containing H statistic, p-value, and degrees of freedom (k-1)
 *
 * @throws std::invalid_argument If there are fewer than 2 groups
...
inline test_result kruskal_wallis_test(const std::vector<std::vector<double>>& groups)
{
    ...
    // Tie correction: H_corrected = H / (1 - sum(t^3 - t) / (N^3 - N))
    {
        std::vector<double> sorted_vals;
        sorted_vals.reserve(total_n);
        for (const auto& p : combined) {
...
/**
 * @brief Perform Fisher's exact test (2x2 contingency table)
 *
 * Tests whether row and column variables are independent in a 2x2 contingency table.
 * Uses exact probability calculation based on hypergeometric distribution,
 * applicable even when expected frequencies are small.
 *
 ...
 * @endcode
 *
 * @param a Frequency of cell (1,1)
 * @param b Frequency of cell (1,2)
 * @param c Frequency of cell (2,1)
 * @param d Frequency of cell (2,2)
 * @param alt Type of alternative hypothesis (default: two-sided)
 * @return test_result Test result containing odds ratio, p-value, and degrees of freedom (NaN)
 *
 * @note Null hypothesis: Row and column variables are independent (odds ratio = 1)
 * @note For two-sided test, sums probabilities of all tables with probability less than or equal to observed
 * @note If b or c is zero, odds ratio is infinity
 */
