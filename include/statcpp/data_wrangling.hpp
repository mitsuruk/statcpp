...
inline std::vector<double> rank_transform(const std::vector<double>& data)
{
    ...
    std::size_t i = 0;
    std::size_t valid_n = valid_indices.size();
    while (i < valid_n) {
        std::size_t j = i;
        // Find elements with the same value
        while (j < valid_n && data[valid_indices[j]] == data[valid_indices[i]]) {
            ++j;
        }
        // Compute average rank
        double avg_rank = (static_cast<double>(i) + static_cast<double>(j) - 1.0) / 2.0 + 1.0;
        for (std::size_t k = i; k < j; ++k) {
            ranks[valid_indices[k]] = avg_rank;
        }
        i = j;
    }
    return ranks;
