    ...
    std::size_t k)
{
    ...
    for (std::size_t c = 1; c < k; ++c) {
        ...
        // Fallback when all data points coincide with existing centroids
        if (total_dist <= 0.0) {
            std::uniform_int_distribution<std::size_t> rand_dist(0, n - 1);
            centroids.push_back(data[rand_dist(rng)]);
            continue;
        }
    ...
    for (iter = 0; iter < max_iter; ++iter) {
        ...
        for (std::size_t c = 0; c < k; ++c) {
            if (cluster_sizes[c] > 0) {
                ...
                // Empty cluster: reinitialize with the farthest data point
                double max_dist = -1.0;
