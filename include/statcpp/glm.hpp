...
struct glm_result {
    ...
    double null_log_likelihood;             ///< Null model log-likelihood
    ...
    double tol = 1e-8)
{
    ...
    double y_mean_original = y_mean;  // Preserve original mean for null deviance
    double eta_init;
    ...
    if (family == distribution_family::gaussian) {
        k += 1.0;  // sigma^2 is also an estimated parameter
    }
