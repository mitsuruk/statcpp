                              ...
                              const std::string& alternative = "two.sided")
{
    ...
    // Standard error under null hypothesis (pooled proportion)
    double p_pool = (p1 + p2) / 2.0;
    double se_null = std::sqrt(2.0 * p_pool * (1.0 - p_pool) / static_cast<double>(n));

    // Standard error under alternative hypothesis
    double se_alt = std::sqrt((p1 * (1.0 - p1) + p2 * (1.0 - p2)) / static_cast<double>(n));
