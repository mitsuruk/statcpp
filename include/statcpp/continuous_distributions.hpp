...
inline double gamma_pdf(double x, double shape, double rate = 1.0)
{
    ...
    if (x == 0.0) {
        ...
        if (shape == 1.0) return rate;  // Exponential distribution PDF at x=0
        return 0.0;  // shape > 1
    }
...
inline double beta_pdf(double x, double alpha, double beta_param)
{
    ...
    if (x == 0.0) {
        ...
        if (alpha == 1.0) return beta_param;  // Beta(1, beta) PDF at x=0
        return 0.0;  // alpha > 1
    }
    if (x == 1.0) {
        if (beta_param < 1.0) return std::numeric_limits<double>::infinity();
        if (beta_param == 1.0) return alpha;  // Beta(alpha, 1) PDF at x=1
        return 0.0;  // beta > 1
    }
