...
auto mode(Iterator first, Iterator last)
{
    ...
    for (auto it = first; it != last; ++it) {
        // Skip NaN values (floating-point types only)
...
/**
 * @brief Weighted harmonic mean (safe range version)
 *
 * Computes the harmonic mean with weights applied to each element.
 * Validates that data and weight ranges have the same size.
 *
 * @tparam Iterator Iterator type
 * @tparam WeightIterator Weight iterator type
 * @param first Begin iterator
 ...
 * @throws std::invalid_argument If range is empty, if ranges differ in size, if negative weight exists, if value is zero, or if sum of weights is zero
 */
template <typename Iterator, typename WeightIterator>
...
/**
 * @brief Weighted harmonic mean (safe range version with projection)
 *
