...
inline one_way_anova_result one_way_anova(const std::vector<std::vector<double>>& groups)
{
    ...
    double p_value = 1.0 - f_cdf(f_statistic, df_between, df_within);
    ...
    const std::vector<std::vector<std::vector<double>>>& data)
{
    ...
    for (std::size_t i = 0; i < a; ++i) {
        ...
        for (std::size_t j = 0; j < b; ++j) {
            ...
            if (n_rep == 0) {
                throw std::invalid_argument("statcpp::two_way_anova: empty cell detected");
            }
            n_total += n_rep;
...
/**
 * @brief Perform Tukey's HSD (Honestly Significant Difference) test (Bonferroni approximation)
 *
 * Performs multiple comparisons among all pairs of groups as a post-hoc test
 * following one-way ANOVA.
 *
 * @note This implementation uses Bonferroni-corrected t-distribution approximation
 *       rather than the true Studentized range distribution. Exact Tukey HSD test
 *       requires the Studentized range distribution (q distribution), but this
 *       approximation provides practically sufficient accuracy.
 *       Results tend to be more conservative (slightly wider confidence intervals,
 *       slightly higher p-values).
 *
 * @param anova_result Result from one-way ANOVA
 * @param groups Data for each group (same as used for computing anova_result)
...
inline posthoc_result tukey_hsd(const one_way_anova_result& anova_result,
                                 ...
                                 double alpha = 0.05)
{
    (void)groups;  // groups is used via anova_result which contains computed statistics
    ...
    for (std::size_t i = 0; i < k; ++i) {
        for (std::size_t j = i + 1; j < k; ++j) {
            ...
            // Standard error using harmonic mean
            double se = std::sqrt(mse * 0.5 * (1.0 / n_i + 1.0 / n_j));

            // Approximation of Studentized range (exact calculation requires a special distribution)
            // Here we use t-distribution approximation
            double q = std::abs(mean_diff) / se;

            // Approximate with Bonferroni-adjusted t-test
            ...
            // Simplified p-value calculation (approximated with t-distribution)
            double p_value = 2.0 * (1.0 - t_cdf(std::abs(mean_diff) / se, df_error));
            p_value = std::min(1.0, p_value * n_comparisons);  // Bonferroni adjustment

            double margin = t_crit * se;
            double lower = mean_diff - margin;
            double upper = mean_diff + margin;
            bool significant = (std::abs(mean_diff) > margin);

            comparisons.push_back({i, j, mean_diff, se, q, p_value, lower, upper, significant});
        }
    }

    return {"Tukey HSD (Bonferroni approximation)", comparisons, alpha, mse, df_error};
}
                                          ...
                                          double alpha = 0.05)
{
    ...
    for (std::size_t i = 0; i < k; ++i) {
        for (std::size_t j = i + 1; j < k; ++j) {
            ...
            double p_value = 2.0 * (1.0 - t_cdf(std::abs(t_stat), df_error));
            p_value = std::min(1.0, p_value * n_comparisons);

            double margin = t_crit * se;
            double lower = mean_diff - margin;
            double upper = mean_diff + margin;
            bool significant = (std::abs(t_stat) > t_crit);
                                       ...
                                       double alpha = 0.05)
{
    ...
    for (std::size_t i = 0; i < k; ++i) {
        for (std::size_t j = i + 1; j < k; ++j) {
            ...
            double p_value = 1.0 - f_cdf(f_stat, df_between, df_error);

            double margin = scheffe_crit * se;
            double lower = mean_diff - margin;
            double upper = mean_diff + margin;
            bool significant = (std::abs(t_stat) > scheffe_crit);
...
{
    return result.factor_a.ss / (result.factor_a.ss + result.error.ss);
}
...
{
    return result.factor_b.ss / (result.factor_b.ss + result.error.ss);
}
...
{
    return result.interaction.ss / (result.interaction.ss + result.error.ss);
}
