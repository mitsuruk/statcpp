...
inline double link_derivative(double mu, link_function link)
{
    switch (link) {
        ...
        case link_function::cloglog: {
            ...
            double log_term = std::log(1.0 - mu);
            // Protection when log(1-mu) is close to 0
            if (std::abs(log_term) < 1e-10) {
                throw std::runtime_error("statcpp::link_derivative: cloglog derivative undefined near mu=0");
            }
            return 1.0 / ((1.0 - mu) * log_term);
        }
        default:
            return 1.0;
    }
    ...
    double tol = 1e-8)
{
    ...
    std::vector<double> z_statistics(p_full);
    ...
    switch (family) {
        ...
        case distribution_family::poisson:
            for (std::size_t i = 0; i < n; ++i) {
                ...
                // lgamma(y+1) is omitted as it's constant for model comparison
            }
            break;
        default:
 ...
 */
inline double pseudo_r_squared_nagelkerke(const glm_result& model, std::size_t n)
{
    double n_d = static_cast<double>(n);
    double ll_null = -model.null_deviance / 2.0;  // Approximation
