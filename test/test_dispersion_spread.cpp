...
/**
 * @brief 基本テスト: 信頼性重み付き分散の計算
 * @test data={2,4,6}, weights={1,2,1} の場合の重み付き分散を検証する
 *
 * 重み付き平均 = (2*1 + 4*2 + 6*1) / (1+2+1) = 16/4 = 4.0
 * 分子 = 1*(2-4)^2 + 2*(4-4)^2 + 1*(6-4)^2 = 4+0+4 = 8
 * 分母 = sum(w) - sum(w^2)/sum(w) = 4 - 6/4 = 2.5
 * 重み付き分散 = 8 / 2.5 = 3.2
 */
TEST(WeightedVarianceTest, BasicReliabilityWeights) {
    std::vector<double> data = {2, 4, 6};
    std::vector<double> weights = {1, 2, 1};
    double result = statcpp::weighted_variance(data.begin(), data.end(),
                                               weights.begin(), weights.end());
    EXPECT_NEAR(result, 3.2, 1e-10);
}

/**
 * @brief 等重みテスト: 全て同じ重みの場合は標本分散と一致する
 * @test 等重みの重み付き分散が sample_variance と等しいことを検証する
 */
TEST(WeightedVarianceTest, EqualWeightsEqualsSampleVariance) {
    std::vector<double> data = {2, 4, 4, 4, 5, 5, 7, 9};
    std::vector<double> weights(data.size(), 1.0);
    double weighted = statcpp::weighted_variance(data.begin(), data.end(),
                                                 weights.begin(), weights.end());
    double sample = statcpp::sample_variance(data.begin(), data.end());
    EXPECT_NEAR(weighted, sample, 1e-10);
}

/**
 * @brief 単一要素テスト: 要素が1つの場合は有効サンプルサイズ不足で例外
 * @test 単一要素で std::invalid_argument が送出されることを検証する
 */
TEST(WeightedVarianceTest, SingleElementThrows) {
    std::vector<double> data = {5.0};
    std::vector<double> weights = {1.0};
    EXPECT_THROW(statcpp::weighted_variance(data.begin(), data.end(),
                                            weights.begin(), weights.end()),
                 std::invalid_argument);
}

/**
 * @brief ゼロ重みテスト: 一部の重みが0の場合でも正しく計算される
 * @test 重み0の要素が結果に影響しないことを検証する
 *
 * data={10, 20, 30}, weights={1, 0, 1}
 * 有効データは {10, 30} (重み {1, 1})
 * 重み付き平均 = (10*1 + 20*0 + 30*1) / (1+0+1) = 40/2 = 20.0
 * 分子 = 1*(10-20)^2 + 0*(20-20)^2 + 1*(30-20)^2 = 100+0+100 = 200
 * 分母 = sum(w) - sum(w^2)/sum(w) = 2 - 2/2 = 1.0
 * 重み付き分散 = 200 / 1.0 = 200.0
 */
TEST(WeightedVarianceTest, ZeroWeightElements) {
...
/**
 * @brief 非推奨3引数版テスト: deprecated な3引数オーバーロードが動作することを検証する
 * @test 3引数版と4引数版で同じ結果が得られることを検証する
 */
TEST(WeightedVarianceTest, DeprecatedThreeArgOverload) {
    std::vector<double> data = {2, 4, 6};
    std::vector<double> weights = {1, 2, 1};

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    double result_deprecated = statcpp::weighted_variance(data.begin(), data.end(),
                                                          weights.begin());
#pragma GCC diagnostic pop

    double result_safe = statcpp::weighted_variance(data.begin(), data.end(),
                                                    weights.begin(), weights.end());
...
/**
 * @brief 基本テスト: 重み付き標準偏差は重み付き分散の平方根
 * @test weighted_stddev が sqrt(weighted_variance) と一致することを検証する
 */
TEST(WeightedStddevTest, IsSqrtOfWeightedVariance) {
    std::vector<double> data = {2, 4, 6};
    std::vector<double> weights = {1, 2, 1};
    double variance = statcpp::weighted_variance(data.begin(), data.end(),
                                                 weights.begin(), weights.end());
    double stddev = statcpp::weighted_stddev(data.begin(), data.end(),
                                             weights.begin(), weights.end());
    EXPECT_DOUBLE_EQ(stddev, std::sqrt(variance));
}

/**
 * @brief 具体値テスト: weighted_stddev の具体的な計算値を検証する
 * @test data={2,4,6}, weights={1,2,1} の重み付き標準偏差 = sqrt(3.2)
 */
TEST(WeightedStddevTest, BasicValue) {
    std::vector<double> data = {2, 4, 6};
    std::vector<double> weights = {1, 2, 1};
    double result = statcpp::weighted_stddev(data.begin(), data.end(),
                                             weights.begin(), weights.end());
    EXPECT_NEAR(result, std::sqrt(3.2), 1e-10);
}
 ...
 * @test 3引数版と4引数版で同じ結果が得られることを検証する
 */
TEST(WeightedStddevTest, DeprecatedThreeArgOverload) {
    std::vector<double> data = {2, 4, 6};
    std::vector<double> weights = {1, 2, 1};

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    double result_deprecated = statcpp::weighted_stddev(data.begin(), data.end(),
                                                        weights.begin());
#pragma GCC diagnostic pop
