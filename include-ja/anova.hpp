...
/**
 * @brief Tukey's HSD（Honestly Significant Difference）検定を実行する（Bonferroni近似）
 *
 * 一元配置分散分析後の事後比較として、全ての群ペア間で多重比較を行います。
 *
 * @note この実装は真の Studentized range 分布ではなく、Bonferroni 補正による
 *       t分布近似を使用しています。正確な Tukey HSD 検定には Studentized range
 *       分布（q分布）が必要ですが、この近似は実用上十分な精度を提供します。
 *       より保守的な結果（やや広い信頼区間、やや高いp値）となる傾向があります。
 *
 * @param anova_result 一元配置分散分析の結果
 * @param groups 各群のデータ（anova_resultの計算に使用したもの）
                                 ...
                                 double alpha = 0.05)
{
    ...
    for (std::size_t i = 0; i < k; ++i) {
        for (std::size_t j = i + 1; j < k; ++j) {
            ...
            // Studentized range の近似 (正確な計算には特殊な分布が必要)
            // ここではt分布による近似を使用
            double q = std::abs(mean_diff) / se;

            // Bonferroni調整されたt検定で近似
            ...
            p_value = std::min(1.0, p_value * n_comparisons);  // Bonferroni調整
