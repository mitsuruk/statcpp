...
inline double gamma_pdf(double x, double shape, double rate = 1.0)
{
    ...
    if (x == 0.0) {
        ...
        if (shape == 1.0) return rate;  // 指数分布の x=0 での PDF
        return 0.0;  // shape > 1
    }
...
inline double beta_pdf(double x, double alpha, double beta_param)
{
    ...
    if (x == 0.0) {
        ...
        if (alpha == 1.0) return beta_param;  // Beta(1, beta) の x=0 での PDF
        return 0.0;  // alpha > 1
    }
    if (x == 1.0) {
        if (beta_param < 1.0) return std::numeric_limits<double>::infinity();
        if (beta_param == 1.0) return alpha;  // Beta(alpha, 1) の x=1 での PDF
        return 0.0;  // beta > 1
    }
...
// ============================================================================
// Studentized Range Distribution（スチューデント化範囲分布）
// ============================================================================

/**
 * @brief スチューデント化範囲分布の CDF
 *
 * k群、自由度dfのスチューデント化範囲分布においてP(Q <= q)を計算します。
 *
...
/**
 * @brief スチューデント化範囲分布の分位関数
 *
 * k群、自由度dfのスチューデント化範囲分布においてP(Q <= q) = pとなるqを計算します。
 *
