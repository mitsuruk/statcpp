 ...
 * @brief MCAR 簡易検定結果
 *
 * MCAR の簡易検定（平均差に基づく）の結果を格納する構造体です。
 * χ²統計量、p値、自由度、および結果の解釈を含みます。
 *
 * @note これは Little (1988) の正式な MCAR 検定とは異なります。
 *       正式な検定には EM アルゴリズムによる共分散推定が必要です。
 */
...
/**
 * @brief MCAR の簡易検定（平均差ベース）
 *
 * 各変数について、他の変数の欠損有無による平均差を t 検定で評価し、
 * t² 統計量を集約してカイ二乗近似により MCAR を検定します。
 *
 * @note これは Little (1988) の正式な MCAR 検定の簡易版です。
 *       正式な検定では EM アルゴリズムによる共分散推定と
 *       マハラノビス距離を使用しますが、この実装では変数間の
 *       相関構造を考慮していません。そのため、正式な検定とは
 *       p 値が異なる場合があります。
 *
    ...
    const std::vector<std::vector<double>>& data)
{
    ...
    for (const auto& row : data) {
        if (row.size() != n_cols) {
            throw std::invalid_argument(
                "statcpp::test_mcar_simple: inconsistent row sizes");
        }
    }
 ...
 * @brief 欠損メカニズムの簡易診断
 *
 * データの欠損メカニズムを簡易的に診断します。
 * 内部で MCAR の簡易検定を実行し、その結果に基づいて
 * MCAR または MAR を返します。
 *
 * @note この関数は MCAR と MAR の 2 分類のみを返します。
 *       MAR と MNAR の区別は観測データだけでは原理的に困難なため、
 *       MCAR が棄却された場合は保守的に MAR と判定します。
 *       missing_mechanism::mnar および missing_mechanism::unknown は
 *       この関数からは返されません。
 *
 * @param data 診断対象の2次元データ（行: 観測、列: 変数）
 * @return missing_mechanism 推定された欠損メカニズム（mcar または mar）
 */
    ...
    unsigned int seed = 0)
{
    ...
    if (m < 2) {
        throw std::invalid_argument(
            "statcpp::multiple_imputation_pmm: m must be >= 2 for Rubin's rules");
    }
    ...
    for (const auto& row : data) {
        if (row.size() != n_cols) {
            throw std::invalid_argument(
                "statcpp::multiple_imputation_pmm: inconsistent row sizes");
        }
    }
    ...
    unsigned int seed = 0)
{
    ...
    if (m < 2) {
        throw std::invalid_argument(
            "statcpp::multiple_imputation_bootstrap: m must be >= 2 for Rubin's rules");
    }
    ...
    for (const auto& row : data) {
        if (row.size() != n_cols) {
            throw std::invalid_argument(
                "statcpp::multiple_imputation_bootstrap: inconsistent row sizes");
        }
    }
