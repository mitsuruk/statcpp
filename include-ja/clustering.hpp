    ...
    linkage_type linkage = linkage_type::single)
{
    ...
    for (std::size_t step = 0; step < n - 1; ++step) {
        ...
        dendrogram.push_back({min_i, min_j, min_dist, cluster_size[new_cluster]});
        ...
        for (std::size_t k = 0; k < new_cluster; ++k) {
            ...
            switch (linkage) {
                ...
                case linkage_type::ward: {
                    ...
                    double nijk = nij + nk;
                    new_dist = std::sqrt(
                        ((ni + nk) * cluster_dist[min_i][k] * cluster_dist[min_i][k] +
                         (nj + nk) * cluster_dist[min_j][k] * cluster_dist[min_j][k] -
                         nk * min_dist * min_dist) / nijk
                    );
                    break;
                }
            }
