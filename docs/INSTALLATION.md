...
### Option 1: Include with namespace prefix (Recommended)
...
This is the recommended approach as it clearly shows which library the headers belong to.

### Option 2: Add statcpp directory to include path

```bash
g++ -std=c++17 -I/usr/local/include/statcpp your_program.cpp -o your_program
```
...
This allows shorter include statements but requires specifying the include path.
