...
This document provides an overview of all modules and public functions in the statcpp library (386 public functions, 538 including overloads, across 31 header files).
...
| `norm_sf` | Standard normal survival function, P(Z > x) |
...
`betainc_impl` and `lgamma_impl` live in the `statcpp` namespace but are the
internal bodies of `betainc` and `lgamma`. They are not part of the intended
public interface and may change without notice.

> `norm_sf` computes the upper tail directly through `erfc`. Forming it as
> `1 - norm_cdf(x)` loses all significance above about x = 8 and returns exactly
> zero from x = 8.30, so use `norm_sf` whenever an upper tail probability is needed.
...
Each distribution provides `_pdf`, `_cdf`, `_quantile`, and `_rand` functions (except studentized range, which provides CDF and quantile only).
